...
def k_reciprocal_neigh( initial_rank, i, k1):   #k相似邻
...
def re_ranking(q_feat, g_feat, k1=20, k2=6, lambda_value=0.3, eval_type=True):
    ...
    original_dist = np.transpose(original_dist / np.max(original_dist, axis=0)) #归一化
    V = np.zeros_like(original_dist).astype(np.float16)

    query_num = q_feat.size(0)
    all_num = original_dist.shape[0]
    if eval_type:
        dist[:, query_num:] = dist.max()    #将边界外的距离设为最大
