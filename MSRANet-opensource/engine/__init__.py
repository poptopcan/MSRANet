...
def get_trainer(dataset, model, optimizer, lr_scheduler=None, logger=None, writer=None, non_blocking=False, log_period=10,
                ...
                eval_interval=None, start_eval=None, start_epoch=0, rerank=False):
    if logger is None:
        logger = logging.getLogger()
        logger.setLevel(logging.WARN)

    # trainer
    trainer = create_train_engine(model, optimizer, non_blocking, start_epoch)
    ...
    def train_start(engine):
        ...
        # setattr(engine.state, 'epoch', start_epoch)
    ...
    def train_completed(engine):
        ...
        if dataset == 'sysu':
            ...
            eval_regdb(q_feats, q_ids, q_cams, g_feats, g_ids, g_cams, g_img_paths, rerank=False)
            print('visible to infrared')
            eval_regdb(g_feats, g_ids, g_cams, q_feats, q_ids, q_cams, q_img_paths, rerank=False)
            if engine.rerank:
                print('rerank:')
                print('infrared to visible')
    ...
    def epoch_completed_callback(engine):
        ...
        if evaluator and epoch % eval_interval == 0 and epoch > start_eval:
            ...
            if dataset == 'sysu':
                perm = sio.loadmat(os.path.join(dataset_cfg.sysu.data_root, 'exp', 'rand_perm_cam.mat'))[
                    'rand_perm_cam']
                mAP, r1, r5, _, _, mINP = eval_sysu(q_feats, q_ids, q_cams, g_feats, g_ids, g_cams, g_img_paths, perm, mode='all', num_shots=1, rerank=rank)
            elif dataset == 'regdb':
                print('infrared to visible')
                mAP, r1, r5, _, _, mINP = eval_regdb(q_feats, q_ids, q_cams, g_feats, g_ids, g_cams, g_img_paths, rerank=engine.rerank)
                print('visible to infrared')
                mAP, r1_, r5, _, _, mINP = eval_regdb(g_feats, g_ids, g_cams, q_feats, q_ids, q_cams, q_img_paths, rerank=engine.rerank)
                r1 = (r1 + r1_) / 2
