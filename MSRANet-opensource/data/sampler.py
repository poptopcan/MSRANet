...
class CrossModalityRandomSampler(Sampler):  #两个模态随机采样图像
    ...
    def __iter__(self):
        ...
        if rgb_size >= ir_size:
            ...
            reps = diff // ir_size      #相差倍率
            pad_size = diff % ir_size   #相差余数
            for _ in range(reps):
...
class CrossModalityIdentitySampler(Sampler):    #两个模态采样随机身份
    def __init__(self, dataset, p_size, k_size):
        ...
        for i, identity in enumerate(dataset.ids):
            if dataset.cam_ids[i] in [3, 6]:
                self.id2idx_ir[identity].append(i)  #id:idx[]字典
            else:
    ...
    def __iter__(self):
        ...
        id_perm = np.random.permutation(self.dataset.num_ids)   #随机排序
        for start in range(0, self.dataset.num_ids, self.p_size):
            selected_ids = id_perm[start:start + self.p_size]

            sample = []
            for identity in selected_ids:
                replace = len(self.id2idx_rgb[identity]) < self.k_size  #判断图像数量，允许重复选择
...
class RandomIdentitySampler(Sampler):   #随机身份采样，随机选择k个身份，每个身份p张图片
...
class NormTripletSampler(Sampler):  #普通PK采样？不考虑模态？
    """
