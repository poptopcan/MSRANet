...
def train(cfg): 
    ...
    def print_parameter_requires_grad(model):
        params_with_grad = [name for name, param in model.named_parameters() if param.requires_grad]
        print("Parameters that requires gradients:")
        for param in params_with_grad:
            print(param)

    print(get_parameter_number(model))
    # print_parameter_requires_grad(model)
...
if __name__ == '__main__':
    ...
    if cfg.resume:
        pattern = r'\\([^\.]+)\.'
        match = re.search(pattern, args.cfg)
        cfg.merge_from_file("configs/"+match+"_resume.yml")
